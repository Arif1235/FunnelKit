...
<div class="fkcart-order-summary fkcart-panel ">
    <div class="fkcart-order-summary-container">
		...
		if ( $shipping_enabled && ! is_null( WC()->cart ) ) {
			$shipping_total = $front->get_shipping_total( true );
			$free_shipping  = false;
			if ( class_exists( 'FKCart\Pro\Rewards' ) && ! is_null( WC()->session ) ) {
				$free_shipping = WC()->session->get( '_fkcart_free_shipping_methods', '' );
			}

			if ( ! empty( $free_shipping ) || $shipping_total > 0 ) {
				?>
                <div class="fkcart-summary-line-item fkcart-shipping-wrap <?php echo ! empty( $free_shipping ) ? 'fkcart-coupon-applied' : ''; ?>">
                    <div class="fkcart-summary-text"><?php _e( 'Shipping', 'woocommerce' ) ?></div>
                    <div class="fkcart-summary-amount">
						<?php
						if ( ! empty( $free_shipping ) ) {
							echo apply_filters( 'fkcart_free_shipping_text', __( 'Free', 'woocommerce' ), $front );
						} else {
							echo wp_kses_post( $front->get_shipping_total() );
						}
						?>
                    </div>
                </div>
